...
$('#footer .status').on('click',function(){
    ...
    $(this)
        .children('.toggle').toggleClass('glyphicon-chevron-down glyphicon-chevron-up')
        .siblings('.stream-activity-filters').slideToggle('fast');
});
...
$('.stream-activity-filters button').on('click',function(){
    ...
    if($(this).is('.activity-all.btn-warning')){ // If Show All Activities is deselected
        ...
        $('.stream-activity-filters-media').removeClass('all-selected');
        ...
        $('.stream-activity-filters-media').addClass('all-selected');
